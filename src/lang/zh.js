export default {
  ...
  text: {
    ...
    confirmDorp2:
      "删除群组数据将删除本地数据库对应的节点数据、区块与交易数据、合约数据等（不影响链上数据）是否确认删除群组数据？",
    confirmStop:
      "请确保当前选中节点所属群组数大于1个，否则停止该节点的群组将导致节点共识异常",
    sure: "确定",
    next: "下一步",
    cancel: "取消",
    select: "请选择",
    ...
    proposalSuccessMsg:
      "修改节点提案成功，如果提案需要多个委员投票，请前往权限管理页面进行投票",
    ...
    sealerListConf:
      '群组创世块配置的共识节点列表 ，例如 : ["nodeId1","nodeId2"]',
    sealerListConf_1:
      '群组创世块配置的共识节点列表 ，加入群组必须与创世块配置的共识节点列表（非当前共识节点列表）一致，例如 : ["nodeId1","nodeId2"]',
    group: "群组",
    ...
    groupConf4_all:
      "群组的本地数据与链上群组数据存在冲突，如重建链或群组，请在群组管理中删除相应群组的数据或新建后台数据库",
    confirm: "确认",
    importSuccessed: "导入成功",
    noExportGroup: "当前群组为同步链上群组所得，无创世块配置",
    agreeNode_1: "新节点加入已有群组包含3个步骤：",
    agreeNode_2:
      "本管理台中若已包含当前群组的节点前置（共识节点/观察节点），可点击下方左侧按钮，提前将新节点加入到当前群组的共识节点/观察节点;",
    agreeNode_3:
      "本平台中无当前群组的节点前置（共识节点/观察节点），则需要用户另行将当前节点",
    agreeNode_4:
      "添加到当前群组的共识节点/观察节点，如已添加，点击“继续”按钮进行下一步操作;",
    agreeNode_5: "生成群组、启动群组、加入群组",
    agreeNode_6: "（共识节点/观察节点）",
    agreeNode_7:
      "详情可参考右上角的帮助文档-WeBASE管理平台使用手册-附录-动态群组管理使用指南",
    ...
    hostListInfo:
      "操作系统要求：CentOS 7.2 / Ubuntu 16.04 / Debian 9;配置要求：单机2核4G，500G硬盘；需要配置ssh免密登录",
    chainName: "链名称",
    chainVersion: "链版本",
    chainStatus: "链状态",
    chainProgress: "链进度",
    ...
    readyAnsibleInfo:
      "需要在WeBASE-Node-Manager所在机器安装Ansible，并为Ansible配置私钥",
    readyDockerInfo: "需要安装最新版Docker并启动Docker，配置docker用户组",
    visualization: "可视化部署流程",
    addHostInfo:
      "进入主机管理，点击新增主机按钮，填入ip和文件路径（文件路径为节点的安装路径）",
    addCheckNode: "添加节点信息并检测、初始化主机",
    addCheckNodeInfo1:
      "进入节点管理，选择镜像拉取方式，选择链版本，点击新增节点按钮添加节点。",
    addCheckNodeInfo2:
      "新增节点弹窗选择主机，填写各个端口，填写节点个数，节点个数即为新增几个节点，其端口根据填写的端口递增。",
    addCheckNodeInfo3:
      "注意：不要填写已使用的端口或者在递增范围内有已使用的端口。",
    checkNodeInfo: "检测主机，新增完节点后，点击检测按钮。",
    initializationInfo:
      "初始化主机，当节点状态为检测成功，可以执行初始化操作，点击初始化按钮。",
    deployStartChain: "部署链并启动",
    deployStartChainInfo:
      "当节点状态为初始化成功，即可进行部署操作，点击部署按钮，部署链是异步操作，时间较长，请耐心等待。",
    nodeLog: "操作日志",
    ...
    appStepTag4: "合约",
    appDescContent:
      "应用管理功能是BAAS管理台提供的一种应用管理插件接入功能——BAAS提供应用管理功能菜单，并提供接入通用性规范。各个应用可以按照接入规范注册到BAAS管理台，以此来实现BAAS和各应用之间从用户，区块链账户，基础配置，合约等方面实现连通。",
    copyInfo: "复制信息",
    imgSize:
      "每次只能上传一张图片,尺寸为80*80px，支持PNG、JPG格式，大小不能超过200KB",
    ...
    versionError:
      "Solc版本加载失败，请检查/static/js目录中是否存在对应的合约版本",
    ...
    disable: "关闭",
    adminAddr: "admin账号",
    configSuccess: "设置成功，请重启节点以使配置生效",
  },
  rule: {
    chainCommite: "链委员已存在",
    eventName: "请输入Event名",
    ...
    contractGroupRule:
      "包含字母，数字和‘.’，以字母开头，不能以“.”结尾,如com.webank",
    IpRule: "请输入正确的IP",
    folderName: "请输入文件夹名称",
  ...
  title: {
    ...
    CNSmanager: "CNS查询",
    CNSTips:
      "CNS查询说明：合约命名服务CNS通过提供链上合约名称与合约地址映射关系的记录及相应的查询功能，方便调用者通过记忆简单的合约名来实现对链上合约的调用。",
    CRUDTips:
      "CRUD说明：CRUD(增删改查)可以创建表，对表进行增删改查操作。Tips：如果启用了部署和建表权限，建表操作需要部署和建表权限。删除和修改表操作需要该表的表权限。",
    ...
    configManager: "配置管理",
    configManagerTips:
      "系统配置管理说明：系统配置可以配置系统属性值（目前支持tx_count_limit和tx_gas_limit属性的设置）。",
    certificate: "证书管理",
  ...
  nodes: {
    ...
    nodeList: "节点列表",
    nodeDescription:
      "节点管理说明：可以通过节点管理设置节点类型。包括：1、根据节点NodeID设置对应节点为共识节点。2、 根据节点NodeID设置对应节点为观察节点。3、根据节点NodeID设置对应节点为游离节点。",
    ...
    guidetep2:
      "选择区块链版本，填写WeBASE-Sign的地址，最后再填写主机信息：包括主机IP，主机部署节点数量，主机所属机构，节点所属群组和区块链数据存储目录。",
  ...
  contracts: {
    ...
    upLoadFile: "上传文件",
    contractTips:
      "群组内部署的合约都需要在合约管理里添加，编译保存，否则会判断为异常合约。",
    rename: "重命名",
  ...
  editor: {
    ...
    InvalidSignature:
      "无效的签名异常，请检查签名的私钥或国密与非国密加密类型是否对应",
    ...
    WASMValidationFailure:
      "WASM二进制验证失败，一般出现在使用了错误的密码学方法、WASM使用了浮点数",
  ...
  privateKey: {
    ...
    importPrivateKeyAccountTip:
      "在启用权限治理模式下，用来添加初始治理委员账户私钥。",
    modifyThresholdTip:
      "投票通过条件 1)满足参与率 2)满足阈值(参与投票权重/总权重)",
    addUserTips:
      "群组内发送交易的账号都需要在私钥管理里添加，否则会判断为异常用户。",
  ...
  system: {
    ...
    node: "节点权限",
    confirmPermission:
      "添加第一个管理员权限的时候，管理员将启动权限，请确认账号是否正确。误操作可能导致服务不可用。",
  ...
  transaction: {
    ...
    stroke: "笔",
    unusualTips:
      "异常过多(大于等于20)，会停止审计。建议查看交易情况，找出异常原因后导入合约或用户来清理异常记录。",
    ...
    endTime: "结束日期",
    unusualMoreTips:
      "异常过多，已经停止审计。建议查看交易情况，找出异常原因后导入合约或用户来清理异常记录。",
  ...
  alarm: {
    ...
    authentication: "鉴权",
    authenticationTips:
      "默认使用username/password进行验证，通过SSL/TLS连接邮箱服务",
    user: "用户",
    password: "密码",
    save: "保存",
    test: "测试",
  ...
  guide: {
    ...
    addFront: "添加节点前置",
    addFrontTip1:
      "节点前置和节点绑定的一个java服务的中间件，集成web3jsdk，webase平台通过该服务连接节点。",
    addFrontTip2: "进入节点管理页面，点击新增节点前置，填写正确的ip和端口。",
    addFrontTip3:
      "注意：首次进入关掉此页面后会出现添加前置弹窗，在此弹窗添加前置即可。",
    addUser: "创建私钥用户",
    addUserTip1: "如果需要部署合约，测试交易，需要创建私钥用户。",
    addUserTip2: "点击私钥管理进入，选择私钥用户，输入用户名。",
    createContract: "创建合约",
    createContractTip1:
      "创建用户后，便可以新增或上传合约，然后编译、部署和发送交易。点击合约管理，合约IDE进入。",
    createContractTip2:
      "点击合约管理，点击合约IDE，点击按钮进行编写或上传合约，然后编译、部署和发送交易。",
  },
  main: {
    changePassword: "修改密码",
    oldPassword: "旧密码",
