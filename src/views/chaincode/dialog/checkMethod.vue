<template>
  <div class="rivate-key-management-wrapper">
    ...
    <div>
      <el-form :model="checkMethodForm" :rules="rules" ref="checkMethodForm" class="demo-ruleForm" label-width="110px">
        ...
        <el-form-item :label="$t('table.contractAbi')" prop="contractAbi">
          <el-input v-model="checkMethodForm.contractAbi" :rows="1" type="textarea" style="width: 300px;"></el-input>
        </el-form-item>
        <!-- <div class="block-wrapper">
                        <el-form-item :label="$t('table.fromBlock')" prop="fromBlock">
                            <el-input v-model.number="checkMethodForm.fromBlock" clearable style="width: 195px;"></el-input>
                        </el-form-item>
                        <el-form-item :label="$t('table.toBlock')" prop="toBlock">
                            <el-input v-model.number="checkMethodForm.toBlock" clearable style="width: 195px;"></el-input>
                        </el-form-item>
                    </div> -->
        <el-form-item :label="$t('table.eventName')" prop="eventName" class="event-option">
          <el-select v-model="checkMethodForm.eventName" :placeholder="$t('placeholder.selected')" style="width:300px;" @change="changeEventName" class="event-name">
        ...
        <el-form-item>
          <div class="text-center" style="width: 500px;">
            <el-button type="primary" @click="submit('checkMethodForm')" :loading="loading">{{$t('text.search')}}</el-button>
...
export default {
  ...
  computed: {
    ...
    eventNameList() {
      var options = [];
      if (!this.checkMethodForm.contractAbi) {
        this.checkMethodForm.eventName = "";
        this.inputList = [];
        return;
      }
      try {
        var abiList = JSON.parse(this.checkMethodForm.contractAbi);
  ...
  methods: {
    ...
    queryContractAbi(queryParam) {
      eventContractInfo(queryParam).then((res) => {
        if (res.data.code === 0) {
          this.checkMethodForm.contractAddress = res.data.data.contractAddress;
          this.checkMethodForm.contractAbi = res.data.data.contractAbi;
        } else {
          this.$message({
            type: "error",
            message: this.$chooseLang(res.data.code),
    ...
    checkMethoding() {
      ...
      var topicObj = {
        eventName: this.checkMethodForm.eventName,
      };
      var indexedList = [];
      ...
      this.loading = true;
      let param = {
        contractAddr: this.checkMethodForm.contractAddress,
        func: this.checkMethodForm.eventName,
        userAddress: this.checkMethodForm.userAddress,
    ...
    logInfo(row) {
      var obj = {
        contractAbi: this.checkMethodForm.contractAbi,
        eventName: this.checkMethodForm.eventName.replace(
          /[(][^）]+[\))]/g,
          ""
        ),
      };
    ...
    decodeEvent(paramVal, dataList) {
      ...
      let contractAbi = JSON.parse(this.checkMethodForm.contractAbi);
      let inputs = [];
      contractAbi.forEach((item) => {
        if (
          item.type == "event" &&
          item.name ===
            this.checkMethodForm.eventName.replace(/[(][^）]+[\))]/g, "")
        ) {
          inputs = item.inputs;
        }
      });
      ...
      return `${this.checkMethodForm.eventName.replace(
        /[(][^）]+[\))]/g,
        ""
      )} (${dataList.join()})`;
    },
    ...
    getBlockNumber: function () {
      getNetworkStatistics(this.groupId)
        .then((res) => {
          ...
          if (status === 200) {
            this.$set(this.checkMethodForm, "toBlock", data.data.latestBlock);
          } else {
            this.$message({
              type: "error",
              message: this.$chooseLang(res.data.code),
