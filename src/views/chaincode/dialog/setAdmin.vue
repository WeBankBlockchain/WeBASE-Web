  ...
  <div>
    <el-form :model="adminData" :rules="rules" ref="adminData" label-width="130px" class="demo-ruleForm">
      ...
      <el-form-item :label="$t('table.eventName')" prop="eventName" class="event-option">
        <el-select v-model="adminData.eventName" :placeholder="$t('placeholder.selected')" style="width:300px;" class="event-name">
...
export default {
  ...
  computed: {
    ...
    eventNameList() {
      ...
      if (!this.adminData.contractAbi) {
        this.adminData.eventName = "";
        this.inputList = [];
        return;
      }
      try {
        var abiList = JSON.parse(this.adminData.contractAbi);
  ...
  methods: {
    ...
    selectAddress(item) {
      console.log(item);
      this.adminData.userAddress = item.address;
    },
    ...
    queryAdminMethod() {
      ...
      queryAdmin(reqData)
        .then((res) => {
          ...
          if (res.data.code == 0) {
            this.adminData.contractAdmin = res.data.data;
          } else {
            this.$message({
