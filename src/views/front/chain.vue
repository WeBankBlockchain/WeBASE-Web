<template>
    <div class="front-module">
        <v-content-head :headTitle="$t('text.chainTitle')" :headSubTitle="$t('text.chainTitle')" ref='heads'></v-content-head>
        <div class="module-wrapper">
            <div class="search-part" style="padding-top: 30px;" v-if='!disabled'>
                <div class="search-part-left">
                    <el-button type="primary" class="search-part-left-btn" :disabled="disabled" @click="onClickAddChain">{{$t('text.addChain')}}</el-button>
        ...
        <div class="module-wrapper" style="margin-top: 10px; padding-bottom: 20px;">
            <div class="search-table">
                ...
                <el-table :data="chainList" class="search-table-content" v-loading="loading">
                    ...
                    <el-table-column :label="$t('text.chainType')" prop="encryptType" show-overflow-tooltip>
                        <template slot-scope='scope'>
                            <span v-if='scope.row.encryptType === 0'>{{$t("text.sha256")}}</span>
                            <span v-if='scope.row.encryptType === 1'>{{$t("text.sm3")}}</span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column :label="$t('devOpsMgmt.operate')">
                        <template slot-scope='scope'>
                            <el-button :disabled="disabled" type="text" size="small" :style="{'color': disabled?'#666':''}" @click="reset(scope.row)">{{$t("dialog.delete")}}</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
...
import {getChainList, deleteChain, getPermissionManagementStatus
} from "@/util/api";
