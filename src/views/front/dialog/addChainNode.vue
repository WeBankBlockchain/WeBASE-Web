    ...
    >
      ...
      >
        ...
        <el-form-item :label="'Front' + $t('alarm.port')" prop="frontPort">
          <el-input
            v-model="nodeFrom.frontPort"
            :placeholder="$t('text.input') + 'Front' + $t('alarm.port')"
            type="number"
            style="width: 240px"
            maxlength="16"
          ></el-input>
        ...
        <el-form-item :label="'P2P' + $t('alarm.port')" prop="p2pPort">
          <el-input
            v-model="nodeFrom.p2pPort"
            :placeholder="$t('text.input') + 'P2P' + $t('alarm.port')"
            type="number"
            style="width: 240px"
            maxlength="16"
          ></el-input>
        ...
        <el-form-item :label="'RPC' + $t('alarm.port')" prop="rpcPort">
          <el-input
            v-model="nodeFrom.rpcPort"
            :placeholder="$t('text.input') + 'RPC' + $t('alarm.port')"
            type="number"
            style="width: 240px"
            maxlength="16"
          ></el-input>
        ...
        <el-form-item :label="$t('nodes.nodeCount')">
          <el-input
            v-model="nodeFrom.nodecount"
            :placeholder="$t('nodes.inputNodes')"
            type="number"
            style="width: 240px"
            maxlength="16"
          ></el-input>
...
export default {
  ...
  methods: {
    ...
    portAbs() {
      ...
      if (
        num1 > this.nodeFrom.nodecount &&
        num2 > this.nodeFrom.nodecount &&
        num3 > this.nodeFrom.nodecount &&
        num4 > this.nodeFrom.nodecount &&
        num5 > this.nodeFrom.nodecount &&
        num6 > this.nodeFrom.nodecount
      ) {
      ...
      if (this.nodeFrom.nodecount > 1) {
        ...
        for (let i = 0; i < list.length; i++) {
          if (this.nodeFrom.hostId == list[i].hostId) {
            if (
              (this.nodeFrom.frontPort < list[i].frontPort &&
                parseInt(this.nodeFrom.frontPort) +
                  parseInt(this.nodeFrom.nodecount) >
                  list[i].frontPort) ||
              this.nodeFrom.frontPort == list[i].frontPort ||
              parseInt(this.nodeFrom.frontPort) +
                parseInt(this.nodeFrom.nodecount) ===
              ...
              this.$message({
                type: "error",
                message: `${this.$t("alarm.port")}${list[i].frontPort}${this.$t(
                  "text.used"
                )}`,
              });
              this.loading = false;
            ...
            if (
              (this.nodeFrom.p2pPort < list[i].p2pPort &&
                parseInt(this.nodeFrom.p2pPort) +
                  parseInt(this.nodeFrom.nodecount) >
                  list[i].p2pPort) ||
              this.nodeFrom.p2pPort == list[i].p2pPort ||
              parseInt(this.nodeFrom.p2pPort) +
              ...
              this.$message({
                type: "error",
                message: `${this.$t("alarm.port")}${list[i].p2pPort}${this.$t(
                  "text.used"
                )}`,
              });
              this.loading = false;
            ...
            if (
              (this.nodeFrom.channelPort < list[i].channelPort &&
                parseInt(this.nodeFrom.channelPort) +
                  parseInt(this.nodeFrom.nodecount) >
                  list[i].channelPort) ||
              this.nodeFrom.channelPort == list[i].channelPort ||
              parseInt(this.nodeFrom.channelPort) +
            ...
            if (
              (this.nodeFrom.rpcPort < list[i].rpcPort &&
                parseInt(this.nodeFrom.rpcPort) +
                  parseInt(this.nodeFrom.nodecount) >
                  list[i].rpcPort) ||
              this.nodeFrom.rpcPort == list[i].rpcPort ||
              parseInt(this.nodeFrom.rpcPort) +
              ...
              this.$message({
                type: "error",
                message: `${this.$t("alarm.port")}${list[i].rpcPort}${this.$t(
                  "text.used"
                )}`,
              });
              this.loading = false;
        ...
        for (let i = 0; i < list.length; i++) {
          if (this.nodeFrom.hostId == list[i].hostId) {
            if (
              list[i].ip == this.nodeFrom.ip &&
              list[i].frontPort == this.nodeFrom.frontPort
            ) {
              ...
              list[i].ip == this.nodeFrom.ip &&
              list[i].p2pPort == this.nodeFrom.p2pPort
            ) {
              this.$message({
                type: "error",
              ...
              list[i].ip == this.nodeFrom.ip &&
              list[i].channelPort == this.nodeFrom.channelPort
            ) {
              this.$message({
                type: "error",
                message: `channel${this.$t("alarm.port")}${this.$t(
                  "text.used"
                )}`,
              });
              this.loading = false;
              ...
              list[i].ip == this.nodeFrom.ip &&
              list[i].rpcPort == this.nodeFrom.rpcPort
            ) {
              this.$message({
                type: "error",
