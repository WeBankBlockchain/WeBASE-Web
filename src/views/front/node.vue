<template>
  <div>
    ...
    <div class="module-wrapper" style="padding-bottom: 20px">
      <p class="wrapper-title" v-if="type != 'node'">
        {{ $t("text.addChain") }}
      </p>
      ...
      <div class="search-part" v-if="type != 'node'">
        ...
        >
          ...
          <el-form-item :label="$t('text.imageMode')" prop="dockerImageType">
            ...
            <el-radio v-model="chainFrom.dockerImageType" :label="0"
              >{{ $t("text.manual")
              }}<el-tooltip
                class="item"
                effect="dark"
                :content="$t('text.imageModeInfo2')"
                placement="top-start"
                ><i
                  class="el-icon-info"
                  style="display: inline-block; padding-left: 10px"
          ...
          <el-form-item :label="$t('text.enableAuth')" prop="enableAuth">
            <el-radio v-model="chainFrom.enableAuth" :label="0"
              >{{ $t("text.disable") }}
            </el-radio>
            <el-radio v-model="chainFrom.enableAuth" :label="1"
              >{{ $t("text.enable") }}
            </el-radio>
            <!-- <el-radio v-model="chainFrom.dockerImageType" :label="3">dockerhub<el-tooltip class="item" effect="dark" :content="$t('text.imageModeInfo3')" placement="top-start"><i class="el-icon-info" style="display: inline-block;padding-left: 10px;"></i></el-tooltip>
                        </el-radio> -->
          ...
          <el-row>
            ...
            <el-col :span="12">
              <el-form-item :label="$t('text.chainStatus') + '：'">
                <span :style="{ color: chainColor(chainFrom.chainStatus) }">{{
                  ChainStatus(chainFrom.chainStatus)
                }}</span>
              </el-form-item>
            </el-col>
            <el-col :span="12">
      ...
      <div class="search-table">
        ...
        >
          ...
          <el-button
            type="primary"
            @click="add"
            v-hasPermi="['bcos3:chain:addNode']"
          ...
          <el-button
            ...
            :loading="loading"
            @click="check()"
            v-hasPermi="['bcos3:chain:checkHostInit']"
            >{{ $t("text.check") }}</el-button
          >
          <el-button class="clear-right" @click="clearNode">{{
            $t("text.clearNode")
          }}</el-button>

          <el-table :data="nodeList" class="search-table-content">
            <el-table-column
              :label="$t('text.hostTitle')"
              prop="ip"
              show-overflow-tooltip
            ></el-table-column>
            <el-table-column
              :label="'Front' + $t('alarm.port')"
              prop="frontPort"
              show-overflow-tooltip
            ></el-table-column>
            <el-table-column
            ...
            >
              <template slot-scope="scope">
                <span :style="{ color: nodeColor(scope.row.status) }">{{
            ...
            >
              <template slot-scope="scope">
                <el-button
                  ...
                  v-hasPermi="['bcos3:chain:deleteNode']"
          ...
          <div style="padding: 10px 0" class="check-button">
            <el-button
              type="primary"
              :loading="loading1"
              @click="init('chainFrom')"
              v-if="!initShow"
              v-hasPermi="['bcos3:chain:initHost']"
              >{{ $t("nodes.initialize") }}</el-button
            >
            <el-button
              type="primary"
              :loading="loading2"
              @click="deploy('chainFrom')"
              v-if="initShow"
              >{{ $t("text.deploy") }}</el-button
            >
          </div>
        </div>
        <div
          style="padding: 30px 0 0 0"
          v-if="nodeList.length && remarkList.length"
        >
    ...
    <addChainNode
      v-if="addChainNodeShow"
      :show="addChainNodeShow"
      :data="addChainNodeData"
      :front="frontList"
      @close="addChainNodeClose"
      @success="addChainNodeSuccess($event)"
    ></addChainNode>
  </div>
</template>
  ...
  methods: {
    ...
    checkone(val) {
      ...
      let data = {
        ...
        agencyName: "agency1",
        ipconf: [
          `${val.ip}:1 agency1 1 ${val.p2pPort},${val.channelPort},${val.rpcPort}`,
        ],
        deployNodeInfoList: [
          {
            hostId: val.hostId,
    ...
    formatParam(type) {
      ...
      for (let i = 0; i < this.nodeList.length; i++) {
        ipconf[
          i
        ] = `${this.nodeList[i].ip}:1 ${agency} ${groupId} ${this.nodeList[i].p2pPort},${this.nodeList[i].channelPort},${this.nodeList[i].rpcPort}`;
      ...
      deployChainData(data)
        .then((res) => {
          ...
          if (res.data.code === 0) {
            ...
            getGroupName(this.chainFrom.groupId)
              .then((res) => {
                if (res.data.code == 0) {
                ...
                this.$router.push({
                  path: "/newNode",
                  query: { enableAuth: this.chainFrom.enableAuth },
                });
              })
              .catch((err) => {
                this.$message({
        ...
        getProgress()
          .then((res) => {
            if (res.data.code === 0) {
              ...
              // 根据进度，修改loading文字描述
              number = number + 1;
              this.laodingText = dynamicPoint(
                this.$t("progress." + this.statusNumber),
                number
              );
              if (number > 1800) {
                clearInterval(this.progressTimer);
