...
export default {
    ...
    mounted() {
        ...
        Bus.$on("chooselanguage", data => {
            ...
            if (!this.data) { return }
            if (this.language = 'zh') {
                this.contractDesc = Base64.decode(data.description);
            } else {
                this.contractDesc = Base64.decode(data.descriptionEn);
            }
        })
        Bus.$on('select1', data => {
            this.codeShow = true;
            ...
            if (this.language = 'zh') {
                this.contractDesc = Base64.decode(this.data.description);
            } else {
                this.contractDesc = Base64.decode(this.data.descriptionEn);
            }
        })
