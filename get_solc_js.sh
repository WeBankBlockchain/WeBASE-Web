...
get_solc_js(){
    ...
    if [ ! -d "${STATIC_JS_DIR}" ];then
        ...
        if [[ ! -f "${STATIC_JS_DIR}/v0.4.24-gm.js" ]]; then
            curl -#L https://osp-1257653870.cos.ap-guangzhou.myqcloud.com/WeBASE/download/solidity/wasm/v0.4.25-gm.js -o "${STATIC_JS_DIR}"/v0.4.24-gm.js
        fi
        if [[ ! -f "${STATIC_JS_DIR}/v0.5.1.js" ]]; then
            curl -#L https://osp-1257653870.cos.ap-guangzhou.myqcloud.com/WeBASE/download/solidity/wasm/v0.5.2.js -o "${STATIC_JS_DIR}"/v0.5.1.js
        fi
        if [[ ! -f "${STATIC_JS_DIR}/v0.5.1-gm.js" ]]; then
            curl -#L https://osp-1257653870.cos.ap-guangzhou.myqcloud.com/WeBASE/download/solidity/wasm/v0.5.2-gm.js -o "${STATIC_JS_DIR}"/v0.5.1-gm.js
        fi
